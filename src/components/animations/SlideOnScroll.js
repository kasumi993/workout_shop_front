...
function SlideOnScroll({ children, animationType = 'slide-bottom', trigger, start = 'top 80%', end = 'bottom 20%', endOpacity = 1, delay = 0, forwardedRef }) {
  ...
  useEffect(() => {
    ...
    if (element.current && typeof window !== 'undefined') {
      ...
      switch (animationType) {
        case 'slide-bottom':
          animationTimeline.fromTo(element.current, { yPercent: -100, opacity: 0 }, { yPercent: 0, opacity: endOpacity, duration: 1, delay, ease: 'power2.out' });
          break;
        case 'slide-top':
          animationTimeline.fromTo(element.current, { yPercent: 100, opacity: 0 }, { yPercent: 0, opacity: endOpacity, duration: 1, delay, ease: 'power2.out' });
          break;
        case 'slide-left':
          animationTimeline.fromTo(element.current, { xPercent: -100, opacity: 0 }, { xPercent: 0, opacity: endOpacity, duration: 1, delay, ease: 'power2.out' });
          break;
        case 'slide-right':
          animationTimeline.fromTo(element.current, { xPercent: +100, opacity: 0 }, { xPercent: 0, opacity: endOpacity, duration: 1, delay, ease: 'power2.out' });
          break;
        case 'fade-in':
          animationTimeline.fromTo(element.current, { opacity: 0 }, { opacity: endOpacity, duration: 1, delay, ease: 'power2.out' });
          break;
  ...
  }, [animationType, trigger, start, end, endOpacity, delay, forwardedRef]);
